...
public class PaymentResource {
    ...
    try {
      ...
      if (!ccDetails.getString("number").startsWith("4")) {
        fail(
          orderId,
          paymentId,
          "Invalid Credit Card: " + ccDetails.getString("number")
        );
      } else {
        pass(
          orderId,
      ...
      fail(
        orderId,
        paymentId,
        "Unknown error: " + ex.getMessage() + " for payment: " + cloudEventJson
      );
    }
  }
  ...
  private void pass(String orderId, String paymentId, String remarks) {
    ...
    producer.send(
      new ProducerRecord<String, String>(paymentsTopic, payload.toString())
    );
  }
