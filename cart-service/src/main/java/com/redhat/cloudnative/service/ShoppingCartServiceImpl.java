...
public class ShoppingCartServiceImpl implements ShoppingCartService {
  ...
  public void priceShoppingCart(ShoppingCart sc) {
    if (sc != null) {
      ...
      if (
        sc.getShoppingCartItemList() != null &&
        sc.getShoppingCartItemList().size() > 0
      ) {
        ...
        for (ShoppingCartItem sci : sc.getShoppingCartItemList()) {
          ...
          sc.setCartItemTotal(
            sc.getCartItemTotal() + sci.getPrice() * sci.getQuantity()
          );
        }
  ...
  void initShoppingCartForPricing(ShoppingCart sc) {
    ...
    for (ShoppingCartItem sci : sc.getShoppingCartItemList()) {
      ...
      if (p != null) {
        sci.setProduct(
          new Product(p.getItemId(), p.getName(), p.getDesc(), p.getPrice())
        );
  ...
  private List<Product> getProducts() {
    ...
    products.add(
      new Product(
        "444437",
        "Lytro Camera",
        "Nanobloc Universal Webcam Cover",
        2.75
      )
    );
  ...
  public ShoppingCart addItem(String cartId, String itemId, int quantity) {
    ...
    if (product == null) {
      log.warn(
        "Invalid product {} request to get added to the shopping cart. No product added",
        itemId
      );
      return cart;
    }
